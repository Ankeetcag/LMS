...
public class EmployeeController {
	...
	public String logincreation(HttpServletRequest request, Model model) throws Exception, IOException {
		...
		if ((userid.equals("GK32960") || userid.equals("GK38924")) && password.equals("Cagl@123")) {
			...
			HttpEntity<String> requestt = new HttpEntity<>(str, headers);
			try {
				HttpEntity<String> response = restTemplate.exchange("https://uatmobile.grameenkoota.in:8010/rpc",
						HttpMethod.POST, requestt, String.class);
				...
				if (jsonObject.has("result")) {
					...
					if (result.has("utUserName")) {
						...
						if (employeeRepository.findById(userid).isPresent()) {
							...
							if (role.contains("BM") || role.contains("KM")) {
								if (session != null) {
